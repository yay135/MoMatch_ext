...
public class SensorService extends Service implements SensorEventListener {
    ...
    private long ORLastTimestamp = 0;
    ...
    public void registorChosenSensors(SensorManager sensorManager, SensorEventListener listener) {
        ...
        Sensor mOrienation = sensorManager.getDefaultSensor(Sensor.TYPE_ORIENTATION);
        ...
        sensorManager.registerListener(listener,mOrienation,SensorManager.SENSOR_DELAY_FASTEST);
        ...
        protected String[] doInBackground(SensorEvent... events) {
            ...
            switch (sensor.getType()) {
                case Sensor.TYPE_ORIENTATION:
                    if((currentTimestamp - ORLastTimestamp) <= 2.2 * sampleInterval) break;
                    else{
                        ORLastTimestamp = currentTimestamp;
                        data[0] = String.valueOf(currentTime+offSet);
                        data[0] = data[0].substring(data[0].length() - 6);
                        data[1] = "5";
                        data[2] = String.format("%.3f",event.values[1]);
                        data[3] = String.format("%.3f",event.values[0]);
                        data[4] = String.format("%.3f",event.values[2]);
                        if (flag) {
                            collected = true;
                            mSensorData.add(data);
                            //Log.e("Orientation", Arrays.toString(data));
                        } else {
                            // tmpData.add(data);
